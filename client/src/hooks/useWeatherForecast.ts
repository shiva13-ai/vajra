...
export type CurrentWeather = {
  ...
  liftedIndex: number;       // Atmospheric instability (< -2 unstable, < -4 severe)
  freezingLevelHeight: number;// Meters AGL (0°C isotherm altitude)
  elevation: number;         // Ground elevation in meters MSL
  marshallPalmerRainRate: number; // mm/hr derived via Z = 200 * R^1.6
};
...
// Determine severity from weather code, convective instability (CAPE, LI) + rain rate
function computeSeverity(
  code: number,
  precipitation: number,
  windGusts: number,
  forecasts: HourlyForecast[],
  currentCape: number = 0,
  currentLI: number = 0
  ...
  const maxCape = Math.max(currentCape, ...forecasts.map((f) => f.cape));
  const minLI = Math.min(currentLI, ...forecasts.map((f) => f.liftedIndex));

  // Severe: Thunderstorm / violent rain / explosive CAPE > 1500 / LI < -4 / extreme gusts
  if (maxForecastCode >= 95 || maxPrecip >= 20 || maxWind >= 80 || maxCape >= 1500 || minLI <= -4) return "severe";
  // Moderate: Rain showers / CAPE > 1000 / LI < -2 / squall gusts
  if (maxForecastCode >= 61 || maxPrecip >= 5 || maxWind >= 50 || maxCape >= 1000 || minLI <= -2) return "moderate";
  // Mild: Drizzle / CAPE > 500 / gusty
  if (maxForecastCode >= 51 || maxPrecip >= 1 || maxWind >= 30 || maxCape >= 500) return "mild";
  return "clear";
}
  ...
  const fetchForecast = useCallback(async (latitude: number, longitude: number) => {
    ...
    try {
      const url =
        ...
        `&hourly=temperature_2m,precipitation,precipitation_probability,wind_speed_10m,wind_gusts_10m,relative_humidity_2m,cloud_cover,weather_code,cape,lifted_index,freezing_level_height` +
      ...
      const currentCape = Number(payload.hourly?.cape?.[0] ?? 0);
      const currentLI = Number(payload.hourly?.lifted_index?.[0] ?? 0);
      const currentFreezingLevel = Number(payload.hourly?.freezing_level_height?.[0] ?? 4200);
      const precip = Number(payload.current?.precipitation ?? 0);

      // Marshall-Palmer derived rain rate R = (Z / 200)^0.625 or measured precip
      const marshallPalmerRainRate = precip > 0 ? precip : 0;
      ...
      const current: CurrentWeather = {
        ...
        freezingLevelHeight: currentFreezingLevel,
        elevation,
        marshallPalmerRainRate,
      };
      ...
      const forecast: HourlyForecast[] = hourlyTimes.slice(1, 7).map((time: string, i: number) => {
        ...
        return {
          ...
          cape: Number(payload.hourly.cape?.[idx] ?? 0),
          liftedIndex: Number(payload.hourly.lifted_index?.[idx] ?? 0),
          freezingLevelHeight: Number(payload.hourly.freezing_level_height?.[idx] ?? 4200),
        };
      });

      const severity = computeSeverity(current.weatherCode, current.precipitation, current.windGusts, forecast, currentCape, currentLI);

      const result: ForecastData = { current, forecast, severity, elevation };
